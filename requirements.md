...
### 運用形態
- **集中管理**: 管制塔1箇所から全医師・全患者を一元的に管理する設計。単一画面・単一DBで、管理者が全操作を行う。

### 対象ユーザー
- 往診業務の管理者（管制塔オペレータ）
- 対応医師（システムは直接操作せず、管制塔からの指示で稼働）
...
### アクセス方式・運用形態
- **集中管理型**: 1箇所の管制塔から全医師・全患者を一元管理
  - 管制塔1台のPCで、全医師のスケジュール・ルート計算・緊急対応を集中して操作
  - 医師側の同時アクセスは想定しない（管制塔が指示を伝達する運用）
...
### 5-1. ユーザーインターフェース
- **単一画面型**: 管制塔用の操作画面
  - 医師・患者情報の入力フォーム（**NG医師**・**希望訪問時間帯**の設定を含む）
  - ルート最適化計算ボタン
...
- **手入力ベース**: 自動位置情報取得は未実装
- **集中管理型**: 管制塔1台からのみ操作。複数ユーザー同時アクセスは非対応（医師数はモック2～3名、本番十数人を想定）
